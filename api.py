...
def _encode(s):
    ...
    if isinstance(s, str):
        s = s.encode(REQUEST_ENCODING).decode(REQUEST_ENCODING)
...
def signature(application_secret, token, params):
    ...
    param_str = "".join(["%s=%s" % (str(key), str(_encode(str(params[key])))) for key in keys])
    param_str += md5((token + application_secret).encode()).hexdigest()
    return md5(param_str.encode()).hexdigest().lower()
...
# Сейчас 2 часа ночи, я ее допилил и исправил косяки. Наканецто